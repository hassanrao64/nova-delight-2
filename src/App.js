...
function AppContent({
  ...
  useEffect(() => {
    ...
    const checkSession = () => {
      const lastActivity = localStorage.getItem('sellerLastActivity');
      if (lastActivity) {
        const lastActivityTime = new Date(lastActivity).getTime();
        const currentTime = new Date().getTime();

        if (currentTime - lastActivityTime > SESSION_TIMEOUT) {
          console.log("Seller session expired due to inactivity");
          handleSellerLogout();
        }
      } else {
        // If for some reason lastActivity is missing but isSeller is true, set it now
        localStorage.setItem('sellerLastActivity', new Date().toISOString());
      }
    };
    ...
    const handleSellerLogout = () => {
      ...
      localStorage.removeItem('sellerLastActivity');
      localStorage.removeItem('sellerStatus');
      localStorage.removeItem('sellerEmail');
      localStorage.removeItem('sellerName');
      localStorage.removeItem('sellerShopName');
      localStorage.removeItem('unreadConversationsCount');
      localStorage.removeItem('sellerLastReset');
    ...
    const resetActivity = () => {
      ...
      const lastReset = parseInt(localStorage.getItem('sellerLastReset') || '0');
      if (now - lastReset > 10000) { // 10 seconds throttle
        localStorage.setItem('sellerLastActivity', new Date().toISOString());
        localStorage.setItem('sellerLastReset', now.toString());
      }
    };
  ...
  const ProtectedSellerRoute = ({ children }) => {
    ...
    // Immediate session expiry check for redirection
    if (lastActivity) {
      const lastActivityTime = new Date(lastActivity).getTime();
      const currentTime = new Date().getTime();
      ...
      if (currentTime - lastActivityTime > SESSION_TIMEOUT) {
        console.log("Protected route caught expired seller session");
        // Clear basic keys to prevent bypass
        localStorage.removeItem('rememberedSeller');
        localStorage.removeItem('sellerId');
