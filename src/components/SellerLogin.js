...
const SellerLogin = ({ setIsSeller }) => {
    ...
    if (userData.role === 'seller') {
      if (userData.status === 'active' || userData.status === 'frozen' || userData.status === 'pending') {
        try {
          ...
          localStorage.setItem('sellerLoginTime', now);
          localStorage.setItem('sellerLastActivity', now);

          // Capture and save login session information for admin view
          await captureLoginSession(userId, userData);
  ...
  const captureLoginSession = async (userId, userData) => {
    try {
      // Get IP and Location information
      let locationData = { ip: 'Unknown', city: 'Unknown', country: 'Unknown' };

      const fetchLocation = async () => {
        // Source 1: ip-api.com (Very reliable city/region data for global IPs)
        try {
        ...
        } catch (e) { console.log('Source 1 (ip-api) failed'); }
        ...
        try {
          ...
          if (resp.ok) {
            const data = await resp.json();
            if (data.city && data.city !== 'Unknown' && data.city !== 'Not found') {
              return {
                ip: data.ip,
                city: data.city,
            ...
            // If city is unknown, store what we have as a baseline
            locationData = { ip: data.ip, city: data.region || 'Unknown', country: data.country_name };
          }
        } catch (e) { console.log('Source 2 (ipapi.co) failed'); }

        // Source 3: Geolocation-DB
        try {
          const resp = await fetch('https://geolocation-db.com/json/');
          if (resp.ok) {
            const data = await resp.json();
            const bestCity = data.city && data.city !== 'Not found' ? data.city : (data.state || 'Unknown');
            if (bestCity !== 'Unknown') {
              return {
        ...
        } catch (e) { console.log('Source 3 (geo-db) failed'); }
      ...
      // Save to Firestore
      await addDoc(collection(db, 'seller_login_sessions'), {
        sellerId: userId,
        sellerName: userData.name || userData.shopName || 'Unknown Seller',
      ...
      console.log('Login session captured successfully');
    } catch (error) {
