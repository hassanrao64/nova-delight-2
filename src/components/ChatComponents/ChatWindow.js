  ...
  const markMessagesAsRead = async () => {
    ...
    try {
      ...
      if (chatDoc.exists()) {
        ...
        const currentCount = chatDoc.data()[fieldToUpdate];

        // Only update if count is > 0 to avoid unnecessary writes
        if (currentCount !== 0) {
        ...
        const messagesRef = collection(db, 'chats', selectedChatId, 'messages');
        const currentUid = currentUserUid || (isAdmin ? localStorage.getItem('adminId') : localStorage.getItem('sellerId'));

        // Query for unread messages sent by the other party
        // We filter for isRead == false
        ...
        if (!unreadSnapshot.empty) {
          ...
          unreadSnapshot.docs.forEach((msgDoc) => {
            ...
            // If the message was NOT sent by the current user, mark it as read
            if (msgData.senderUid !== currentUid) {
    ...
    <ChatWindowContainer>
          ...
          <>
            {messages.map((message) => (
              <Message
                ...
                message={message}
                isAdmin={isAdmin}
                onDeleteMessage={isAdmin && !isMiniAdmin ? confirmDeleteMessage : undefined}
              />
            ))}
      ...
      <InputContainer>
        <ChatInput
          ...
          disabled={isMiniAdmin || (!isAdmin && sellerStatus === "inactive")}
        />
      </InputContainer>
