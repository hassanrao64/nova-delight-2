...
const AdminDashboard = ({ adminRole }) => {
  ...
  const [sellerLoginSessions, setSellerLoginSessions] = useState([]);
  ...
  useEffect(() => {
    if (activeTab === 'storehouse' || activeTab === 'products') {
      ...
      fetchOrders();
    } else if (activeTab === 'sellerLogins') {
  ...
  const fetchSellerLoginSessions = async () => {
    try {
      setLoading(true);
      const sessionsRef = collection(db, 'seller_login_sessions');
      ...
      setSellerLoginSessions(sessionsData);
    } catch (error) {
      console.error('Error fetching seller login sessions:', error);
      setSnackbar({
        open: true,
        message: 'Error fetching login sessions',
        severity: 'error'
      });
    } finally {
      setLoading(false);
    ...
    if (activeTab === 'dashboard') {
      ...
      return (
        <Box sx={{ width: '100%', p: 3 }}>
          ...
          <SectionCard
            title="Recent Seller Logins"
            action={
              <Button
                startIcon={<RefreshIcon />}
                variant="outlined"
                size="small"
                onClick={fetchSellerLoginSessions}
                disabled={loading}
              >
                Refresh
              </Button>
          ...
          >
            <TableContainer component={Paper} sx={{ borderRadius: 4, boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: '1px solid #eef0f7', overflow: 'hidden' }}>
              <Table>
                <TableHead sx={{ bgcolor: '#f8faff', borderBottom: '2px solid #edf0f5' }}>
                  <TableRow>
                    <TableCell sx={{ fontWeight: 'bold', color: '#1a237e' }}>Seller Info</TableCell>
                    <TableCell sx={{ fontWeight: 'bold', color: '#1a237e' }}>Device & Browser</TableCell>
                    <TableCell sx={{ fontWeight: 'bold', color: '#1a237e' }}>Location & IP</TableCell>
                    <TableCell sx={{ fontWeight: 'bold', color: '#1a237e' }}>Login Time</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {sellerLoginSessions.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} align="center" sx={{ py: 6 }}>
                        {loading ? (
                          ...
                          <Box sx={{ textAlign: 'center', color: 'text.secondary' }}>
                            <AssignmentIndIcon sx={{ fontSize: 48, mb: 1, opacity: 0.3 }} />
                            <Typography>No login sessions recorded yet</Typography>
                          </Box>
                        )}
                      </TableCell>
                    </TableRow>
                  ) : (
                    sellerLoginSessions.map((session) => (
                      <TableRow key={session.id} hover sx={{ '&:hover': { bgcolor: '#fcfdff' } }}>
                        <TableCell>
                          <Typography variant="subtitle2" fontWeight="bold" sx={{ color: '#2c3e50' }}>{session.sellerName}</Typography>
                          <Typography variant="body2" color="textSecondary">{session.email}</Typography>
                        </TableCell>
                        <TableCell>
                          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            ...
                            <Tooltip title={session.userAgent}>
                              <Typography variant="caption" color="textSecondary" sx={{ maxWidth: 220, display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                {session.userAgent}
                              </Typography>
                            </Tooltip>
                          </Box>
                        ...
                        <TableCell>
                          <Typography variant="body2" fontWeight="medium">{session.location}</Typography>
                          <Typography variant="caption" sx={{ color: '#7f8c8d' }}>IP: {session.ip}</Typography>
                        </TableCell>
                        <TableCell>
        ...
        >
          <Box sx={{ overflow: 'auto' }}>
            ...
            <List>
              ...
              <ListItemButton
                selected={activeTab === 'sellerLogins'}
                onClick={() => handleTabChange('sellerLogins')}
                sx={{
                  '&.Mui-selected': { backgroundColor: '#edf3fd', color: '#3b82f6' },
                  '&.Mui-selected:hover': { backgroundColor: '#e5effd' },
                  borderRadius: '4px',
              ...
              >
                <ListItemIcon>
                  <AssignmentIndIcon color={activeTab === 'sellerLogins' ? 'primary' : 'inherit'} />
                </ListItemIcon>
                <ListItemText primary="Seller Logins" />
              </ListItemButton>
